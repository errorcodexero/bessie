...
void loop() {
  ...
  for (uint8_t i = 0; i < length; i++) {
    if (PS3[i]->PS3Connected || PS3[i]->PS3NavigationConnected) {
      if (PS3[i]->getAnalogSwitch(LeftSwitchX) > 137 || PS3[i]->getAnalogSwitch(LeftSwitchX) < 117 || PS3[i]->getAnalogSwitch(LeftSwitchY) > 137 || PS3[i]->getAnalogSwitch(LeftSwitchY) < 117 || PS3[i]->getAnalogSwitch(RightSwitchX) > 137 || PS3[i]->getAnalogSwitch(RightSwitchX) < 117 || PS3[i]->getAnalogSwitch(RightSwitchY) > 137 || PS3[i]->getAnalogSwitch(RightSwitchY) < 117) {
        Serial.print(F("\r\nLeftSwitchX: "));
        Serial.print(PS3[i]->getAnalogSwitch(LeftSwitchX));
        Serial.print(F("\tLeftSwitchY: "));
        Serial.print(PS3[i]->getAnalogSwitch(LeftSwitchY));
        if (PS3[i]->PS3Connected) { // The Navigation controller only have one joystick
          Serial.print(F("\tRightSwitchX: "));
          Serial.print(PS3[i]->getAnalogSwitch(RightSwitchX));
          Serial.print(F("\tRightSwitchY: "));
          Serial.print(PS3[i]->getAnalogSwitch(RightSwitchY));
        }
      }
