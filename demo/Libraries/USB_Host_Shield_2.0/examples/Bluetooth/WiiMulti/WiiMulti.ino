  ...
  for (uint8_t i = 0; i < length; i++) {
    ...
    if (Wii[i]->nunchuckConnected) {
      ...
      if (Wii[i]->getButtonClick(C))
        Serial.print(F("\r\nC"));
      if (Wii[i]->getAnalogSwitch(SwitchX) > 137 ||  Wii[i]->getAnalogSwitch(SwitchX) < 117 || Wii[i]->getAnalogSwitch(SwitchY) > 137 || Wii[i]->getAnalogSwitch(SwitchY) < 117) {
        Serial.print(F("\r\nSwitchX: "));
        Serial.print(Wii[i]->getAnalogSwitch(SwitchX));
        Serial.print(F("\tSwitchY: "));
        Serial.print(Wii[i]->getAnalogSwitch(SwitchY));
      }
    }
  }
}
