...
void loop() {
  ...
  if (Wii.nunchuckConnected) {
    ...
    if (Wii.getButtonClick(C))
      Serial.print(F("\r\nC"));
    if (Wii.getAnalogSwitch(SwitchX) > 137 ||  Wii.getAnalogSwitch(SwitchX) < 117 || Wii.getAnalogSwitch(SwitchY) > 137 || Wii.getAnalogSwitch(SwitchY) < 117) {
      Serial.print(F("\r\nSwitchX: "));
      Serial.print(Wii.getAnalogSwitch(SwitchX));
      Serial.print(F("\tSwitchY: "));
      Serial.print(Wii.getAnalogSwitch(SwitchY));
    }
  }
#endif
}
