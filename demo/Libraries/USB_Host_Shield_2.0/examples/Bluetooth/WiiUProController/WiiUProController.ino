...
void loop() {
  ...
  if (Wii.wiiUProControllerConnected) {
    ...
    if (Wii.getAnalogSwitch(LeftSwitchX) > 2200 || Wii.getAnalogSwitch(LeftSwitchX) < 1800 || Wii.getAnalogSwitch(LeftSwitchY) > 2200 || Wii.getAnalogSwitch(LeftSwitchY) < 1800 || Wii.getAnalogSwitch(RightSwitchX) > 2200 ||  Wii.getAnalogSwitch(RightSwitchX) < 1800 || Wii.getAnalogSwitch(RightSwitchY) > 2200 || Wii.getAnalogSwitch(RightSwitchY) < 1800) {
      Serial.print(F("\r\nLeftSwitchX: "));
      Serial.print(Wii.getAnalogSwitch(LeftSwitchX));
      Serial.print(F("\tLeftSwitchY: "));
      Serial.print(Wii.getAnalogSwitch(LeftSwitchY));
      Serial.print(F("\tRightSwitchX: "));
      Serial.print(Wii.getAnalogSwitch(RightSwitchX));
      Serial.print(F("\tRightSwitchY: "));
      Serial.print(Wii.getAnalogSwitch(RightSwitchY));
    }
  }
}
