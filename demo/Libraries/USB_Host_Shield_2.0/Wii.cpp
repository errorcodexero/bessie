                                                ...
                                                if(motionPlusConnected) {
                                                        if(l2capinbuf[20] & 0x02) { // Check if it's a report from the Motion controller or the extension
                                                                ...
                                                                if(nunchuckConnected) {
                                                                        hatValues[StickX] = l2capinbuf[15];
                                                                        hatValues[StickY] = l2capinbuf[16];
                                                        ...
                                                        hatValues[LeftStickX] = (l2capinbuf[15] | l2capinbuf[16] << 8);
                                                        hatValues[RightStickX] = (l2capinbuf[17] | l2capinbuf[18] << 8);
                                                        hatValues[LeftStickY] = (l2capinbuf[19] | l2capinbuf[20] << 8);
                                                        hatValues[RightStickY] = (l2capinbuf[21] | l2capinbuf[22] << 8);
                                                }
#endif
                                                break;
#ifdef DEBUG_USB_HOST
...
uint8_t WII::getAnalogStick(StickEnum a) {
        if(!nunchuckConnected)
...
uint16_t WII::getAnalogStick(AnalogStickEnum a) {
        if(!wiiUProControllerConnected)
                return 2000;
        else {
